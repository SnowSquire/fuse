...
const {
	...
	unconfigure,
	isConfigured,
} = require(join(__dirname, "lib", target, "index.js"));

const IS_OSX = _platform() === "darwin";
const IS_WIN32 = _platform() === "win32";
const DEFAULT_OSX_VOLICON =
...
class Fuse extends Nanoresource {
	...
	_fuseOptions() {
		...
		// Windows & OSX only.
		if (this.opts.volname && (IS_WIN32 || IS_OSX))
			options.push(`volname=${this.opts.volname}` || basename(this.mnt));

		// OSX only.
		if (this.opts.volicon && IS_OSX) {
			const iconPath =
				this.opts.volicon === true ? DEFAULT_OSX_VOLICON : this.opts.volicon;
	...
	_makeHandlerArray() {
		...
		function makeHandler(name, op, defaults, nativeSignal) {
			...
			return (nativeHandler, opCode, ...args) => {
			...
			function signal(nativeHandler, error, ...args) {
				let arr = [nativeHandler, error, ...args];
	...
	_open(cb) {
		...
		if (this._force) {
			return _stat(join(this.mnt, "test"), (error, st) => {
		...
		function open() {
			...
			return _stat(self.mnt, (error, stat) => {
				if (error) {
					return cb(error);
				}
				if (!stat.isDirectory()) {
...
function getStatfsArray(statfs) {
	...
	ints[0] = statfs?.bsize || 0;
	ints[1] = statfs?.frsize || 0;
	ints[2] = statfs?.blocks || 0;
	ints[3] = statfs?.bfree || 0;
	ints[4] = statfs?.bavail || 0;
	ints[5] = statfs?.files || 0;
	ints[6] = statfs?.ffree || 0;
	ints[7] = statfs?.favail || 0;
	ints[8] = statfs?.fsid || 0;
	ints[9] = statfs?.flag || 0;
	ints[10] = statfs?.namemax || 0;
...
function getStatArray(stat) {
	...
	ints[1] = stat?.uid || 0;
	ints[2] = stat?.gid || 0;
	setDoubleInt(ints, 3, stat?.size || 0);
	ints[5] = stat?.dev || 0;
	ints[6] = stat?.nlink || 1;
	ints[7] = stat?.ino || 0;
	ints[8] = stat?.rdev || 0;
	ints[9] = stat?.blksize || 0;
	setDoubleInt(ints, 10, stat?.blocks || 0);
	setDoubleInt(ints, 12, toDateMS(stat?.atime));
	setDoubleInt(ints, 14, toDateMS(stat?.mtime));
	setDoubleInt(ints, 16, toDateMS(stat?.ctime));
